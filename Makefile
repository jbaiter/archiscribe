...
$(BIN)/packr: | $(BASE) ; $(info $(M) building packr…)
	$Q go get github.com/gobuffalo/packr/packr
...
packr: vendor $(shell find client/src -type f) | $(BASE) $(PACKR) ; $(info $(M) running packr…) @ ## Generate packr package
	$Q $(PACKR) -z -i ./web
...
# Client-side code
client/node_modules: ; $(info $(M) installing client dependencies…) @
	$Q cd client && yarn install --silent

client/dist: $(shell find client -type f) client/node_modules ; $(info $(M) building client-side code…) @
	$Q cd client && yarn -s run build --silent
